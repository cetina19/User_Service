...
from rest_api.main import register_user  # Adjust the import based on your actual module structure
from sqlalchemy.orm import Session
from fastapi import Depends
from rest_api.models.users import UserCreate  # Adjust based on your actual model location
from rest_api.services.smtp import get_email_service  # Adjust based on your actual service location
from rest_api.database import get_db  # Adjust based on your actual database module
...
@pytest.fixture
def user_data():
    return {'name': 'existing_user', 'email': 'alper8540@gmail.com', 'password': 'securepassword', 'age': 25}
...
def user(user_data, email, password, name, age):
    user_data['name'] = name
    user_data['age'] = age
...
@given('a database that does not have the user "<username>"')
def db_without_user(mock_db, username):
    mock_db.query.return_value.filter.return_value.first.return_value = None

@given('a database that already has the user "existing_user"')
def db_with_user(mock_db):
    mock_db.query.return_value.filter.return_value.first.return_value = {'username': 'existing_user'}
